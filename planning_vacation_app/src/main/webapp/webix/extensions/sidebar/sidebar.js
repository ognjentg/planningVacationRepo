...
webix.protoUI({
    ...
    _initSidebar: function () {
        ...
        this.attachEvent("onItemRender", function (obj, index) {
            var that = this;
            setTimeout(function () {
                if (obj.type == "separator") {
                    that.addCss(obj.id, "menu_separator_item");
                } else that.addCss(obj.id, "menu_normal_item");
            }, 10);
        });
        ...
        this.attachEvent("onItemClick", function (id, ev, node) {
            if (this.getItem(id).type == "separator") {
                return false;
            }
    ...
    _showPopup: function (id, node) {
        if (this.config.collapsed && this.getItem(id).type != "separator") {
        ...
        } else if (this.getItem(id).type == "separator") {
            this.getPopup().hide();
        }
    },
    ...
    _initContextMenu: function () {
                ...
                body: {
                    rows: [
                        ...
                        },
                        {
                            ...
                            type: {
                                height: 41,
                                css: "menu-popup-list-item"//  dimensions of each dataview item
                            },
                            on: {
        ...
        var h = webix.event(document.body, "mousemove", function (e) {
            ...
            //ALTERNATE: Class name depndent of sidebar
            // var trgParentClassName = trg.parentElement.className || null;
            // if(!popup.config.hidden && !popup.$view.contains(trg) && !master.$view.firstChild.contains(trg) && trgParentClassName.indexOf("sidebar")<0 && trgParentClassName.indexOf("scroll")<0){
            if (!popup.config.hidden && !master.$view.contains(trg) && !popup.$view.contains(trg)) {
                popup.hide();
            }
        });
    ...
    collapsed_setter: function (value) {
        ...
        if (!value) {
            ...
            webix.html.removeCss(this.$view, "webix_sidebar_collapsed", true);
        }
        else {
            this.type.collapsed = true;
            webix.html.removeCss(this.$view, "webix_sidebar_expanded");
            webix.html.addCss(this.$view, "webix_sidebar_collapsed", true);
        }

        this.define("width", width);
        this.resize();

        this.callEvent("onMenuViewChanged", []);
...
webix.type(webix.ui.tree, {
    ...
    template: function (obj, common) {
        ...
        if (common.collapsed) {
            ...
            if (obj.type == "separator") {
                return '<div class="menuSeparator">' + obj.value + "</div>";
            }
