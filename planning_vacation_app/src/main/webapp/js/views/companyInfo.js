...
var companyInfoView = {
        ...
        connection.sendAjax("GET",
            ...
            function (text, data, xhr) {
                if (text) {
                    ...
                    startedSelectedValues = $$("nonWorkingDaysInWeek").getValue().split(",").filter(function (s) {
                        return s;
                    }).map(function (s) {
                        return parseInt(s)
                    })
    ...
    addCollectiveVacation: function () {
        ...
        var dateFromInDTFormat = webix.Date.dateToStr("%d.%m.%Y.")(dateFromValue);
        var dateToInDTFormat = webix.Date.dateToStr("%d.%m.%Y.")(dateToValue);
        var companyId = userData.companyId;
        if ("" == dateFromValue || "" == dateToValue)
            ...
            $$("collectiveVacationDT").eachRow(function (row) {
                ...
                if (dateFromInDTFormat == record.dateFrom && dateToInDTFormat == record.dateTo)
                    isDaySelected = 1;
            });
